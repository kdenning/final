...
average_imdb <- function(x) {
 ...
 #I  havent used this aggregate function before, but it looks really cool for summarizing the elements you are interested in 
...
df_genre <- data %>% 
  drop_na()%>%
  #good use of drop NA, I just learned this recently and it's been a huge help
  group_by(genres)%>%
  nest()
...
#good use of map to compare two different groups, you could even write a function that compares these two groups side-by-side or takes the difference
```
...
```{r}
#Are you going to do anything with the country variable or just using it as an example?
#let's separate the combination of genres for each movie and keep the first genre as main genre in a new variable
#do you know how the first variable is placed before the second variable, is it alphabetical or is there a system behind the order of the genres?

data_new <- data%>%
 drop_na() %>%
#what are you filtering out here? Movies that didn't get any likes on Facebook? Is this common?
 filter(movie_facebook_likes != "0")


data_new<- cSplit(data_new, "genres", "|")
#I haven't seen this cSplit function before but it looks perfect for your situation, if you want to stay in the tidyverse you could probably do this with separate function
...
#not sure if this is just me, but I received an error that object profit could not be found, I can see it in df_main_genre, but my R is having a hard time accesssing it. 
...
#everything worked with this second version, so I agree this is a better choice
...
#Awesome use of map2 to help you with your model comparison!
...
#these plots look great, make sure to remind us that this is comparing model fit and what variables we are comparing, since we've take mandy steps since the original question was posed. 
# I think it makes sense to keep the plots in alphabetical order but you could also arrange by model fit
```
...
#You might want to increase dot size here to make it a little more visible    
