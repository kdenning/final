...
# Great job here! Could add a meaningful error message for when there is no actor who
# matches the query or the actor name is misspelled
```
...
# This is another helpful, straightforward function - good job! Could consider making the
# variable names more meaningful
```
...
# I wonder if there's a programmatic way to select the genre columns you want/don't want?
...
```{r}
# The below line throws an error ("object 'profit' not found")
...
# It's hard to track what the models are fitting and what the graph is supposed to show.
# I know y'all will flesh this out in the tutorial, so I'm not too worried about it :)
```
