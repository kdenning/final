...
library(here)
library(stringr)
#install.packages("splitstackshape") #AAC: I liked that you noted that I might have to install this package
...
data <- import(here("data", "movie_metadata.csv_movie_metadata.csv")) #AAC: Great use of heere library
...
#I might consider using chunk options to not print errors/messages
```
...
#AAC: Good use of description and annotation

#AAC: Good function use! I learned the aggregate function.
...
#AAC: I love that you tested it, and great meaningful function name
```

```{r}

#AAC: I would give this chunk a meaningful name

# let's nest our data by genre

df_genre <- data %>% 
  drop_na() %>% #AAC: I love this drop na function!
  group_by(genres) %>%
  nest()
...
df_main_genre <- data_new %>%
 ...
 filter(!grepl('Thriller|Western|Musical|Family|Romance|Sci-Fi', main_genre)) %>% #AAC: What is grepl?
...
#AAC: I would give this chunk a meaningfuk name
...
#AAC: I like that you recorded your thought process. However, this chunk gave me an error and said "profit" was not found. Weird. 
...
#AAC: I liked that you used map to compare moddels. I know we talked about it in class, but I wasn't sure what it really would look like until I saw your code. Thanks!
...
##AAC: However, this chunk gave me an error and said "by_main_genre" was not found. Weird. 
...
#AAC: I really like how you included a section on saving plots -I feel like this is always the most confusing part for me! 
