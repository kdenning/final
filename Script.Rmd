...
library(glue)
library(distill)
# added this line here because my R needs this to run : )
...
# Great and detailed comments on the environment!
...
```{r}
# maybe more helpful to name the chunks
...
# maybe change this chuck's setting to hide immediate output if we don't need them there for the final script
```
...
#We will use pmap because it will allow us to loop through two or more vectors. In this case we will only be looping through 2, but if we decided to loop through 3 we could add a label for each of the. When using pmap you need to first say what the list is; in our case we will put pmap(list(data, genre). Now they are in a list we can refer to them in their position. For example, data can be referred to as ..1, and genre will be ..2. In order to create each plot we will use mutate and save it to "plot". From there we just ggplot like we regularly would. We will put ..1 as our data input because that says pull from the data list column.
