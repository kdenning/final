...
#Named the columns of the descriptives table and changed the style of code a bit. 
...
#This is cool1 I did not know about %<>% before. 
...
#Bellow I am suggesting that you can probably use the nesting by genre to produce some visualization. I was personally curious about the number of facebook likes that movies get across time and also if that is related to their imdb score. That's the reason I am suggesting the following plots. Also using at least one instance of parallel iterations is part of the requirements of the final product. This code will help meet this requirement. 
 
df_main_genre_plots <- data_new %>%
 group_by(main_genre) %>%
 nest() %>%
 mutate(plot = map2(data, main_genre, ~
      ggplot(.x, aes(title_year, movie_facebook_likes)) +
          geom_point(aes(col = imdb_score)) +
        scale_color_viridis_c()+
          labs(x = "",
            y = "Number of Facebook likes",
            title = glue("Number of Facebook likes: {.y}"))+
        theme_minimal()))
  

df_main_genre_plots$plot
